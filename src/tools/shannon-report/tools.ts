...
export function createShannonReport(): ToolDefinition {
  return tool({
    ...
    async execute(args) {
      ...
      const correlate = args.correlate ?? false
      const timestamp = new Date().toISOString()

      let finalFindings = args.findings

      if (correlate) {
        try {
          const rawFindings: RawFinding[] = JSON.parse(args.findings)
          if (Array.isArray(rawFindings)) {
            const correlated = rawFindings.map((f) => {
              ...
              const severity = normalizeSeverity(f.severity_hint, mapping.cvss_base)
...
function formatCorrelatedFindings(findings: any[]): string {
  ...
  for (const f of findings) {
    if (!seenOwasp.has(f.owasp_id)) {
      ...
      output.push(`| ${f.owasp_id} | ${f.owasp_category} | ${owaspCounts[f.owasp_id]} |`)
    }
  }
  output.push("")
